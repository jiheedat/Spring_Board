	...
	$(function(){ //아이디 중복체크
		$("#id").keyup(function(){
			...
			if(id.length < 4 ) {
				...
				$("#idCheckDiv").text("4자이상 16자 이하의 영문자와 숫자로 작성해주세요");
				return;
			}
			if(id.length > 16) {
				/* $("#idCheckDiv").css("display","block"); */
				$("#idCheckDiv").removeClass("alert-success");
				$("#idCheckDiv").addClass("alert-danger");
				$("idCheckDiv").text("16자 이내로 작성해주세요");
				return;
			}
		});
		$("#id").on('blur',function(){
...
<body>
	...
	<div class = "container">
		...
		<div class ="col-lg-4">
			<div class = "font jumbotron" id="join_top">
				<form action="Register" method="post" enctype="multipart/form-data" onsubmit="return checkSubmit();">
					<h3 id="sign_in">Sign in</h3>
						<div class = "form-group">
							<label for="id"> 아이디 </label>
							<input type = "text" class = "form-control" placeholder= "4~12자의 영문 대소문자와 숫자로만 입력 " name = "id"  id="id" minlength = "4" required>
							<div class="alert alert-danger" id="idCheckDiv"></div><!-- 글자수 체크 -->
							<span id="idError" class="overlap"></span> <!-- 중복체크 -->
						</div>
						<div class = "form-group">
					...
					[ 프로필 사진 선택 ]  
					<input type="file" name="file" id="profil">
